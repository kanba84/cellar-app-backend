...
func (s *Service) ListBottles() ([]model.Bottle, error) {
	query := `
		SELECT 
			...
			w.id, w.name, w.vintage, w.wine_type_id, wt.name as wine_type_name, w.country_id, c.name as country_name, c.iso_code as country_iso_code, w.region_id, r.name as region_name, w.producer, w.label_image_url
		FROM bottles b
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&wine.ID, &wine.Name, &wine.Vintage, &wine.WineTypeID, &wine.WinTypeName, &wine.CountryID, &wine.CountryName, &wine.CountryISOCode, &wine.RegionID, &wine.RegionName, &wine.Producer, &wine.LabelImageURL,
		); err != nil {
			return nil, err
		}
