...
func (h *Handler) CreateWineWithBottle(c *gin.Context) {
	...
	// LabelImageURLがnilの場合は空文字列で初期化
	if req.Wine.LabelImageURL == nil {
		emptyStr := ""
		req.Wine.LabelImageURL = &emptyStr
	}
	...
	if strings.HasPrefix(contentType, "multipart/form-data") {
		...
		if err == nil && file != nil {
			...
			go func(file *multipart.FileHeader, wineID uint, fileName string) {
				...
				url := fmt.Sprintf("https://cellar-app.local/labels/%s", fileName)
				if err := h.Service.PatchWine(wineID, map[string]interface{}{"label_image_url": url}); err != nil {
					log.Printf("async PatchWine label update error: %v", err)
				}
...
func generateJPEGFileName() string {
	...
	result := make([]byte, lenRand)
	for i := range result {
		result[i] = letters[rand.Intn(len(letters))]
	}
	return "thumb_" + string(result) + ".jpg"
}
